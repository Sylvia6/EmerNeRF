...
def main(args):
    ...
    model = builders.build_composed_model_from_cfg(
    ...
    for step in metric_logger.log_every(all_iters, cfg.logging.print_freq):
        ...
        if step > 0 and (
            cfg.data.pixel_source.load_rgb
            ...
            if cfg.data.pixel_source.sampler.buffer_ratio > 0:
                with torch.no_grad():
                    ...
                    if isinstance(dataset.pixel_source, dict):
                        for _, pixel_source in dataset.pixel_source.items():
                            pixel_source.update_downscale_factor(
                    ...
                    if isinstance(dataset.pixel_source, dict):
                        ...
                        s, e = 0, 0
                        for _, pixel_source in dataset.pixel_source.items():
                            l = pixel_source.num_imgs
                            s, e = e, e+l
                            pixel_source.reset_downscale_factor()
                            pixel_source.update_pixel_error_maps(render_results, idx=[s, e])
                            maps_video.extend(pixel_source.get_pixel_sample_weights_video())
                    else:
                        dataset.pixel_source.reset_downscale_factor()
                    ...
                    for i in range(len(maps_video) // dataset.num_cams):
                        frames = maps_video[
                            i
                            * dataset.num_cams : (i + 1)
                            * dataset.num_cams
                        ]
                        frames = [
        ...
        if step > 0 and step % cfg.logging.vis_freq == 0:
            ...
            if cfg.data.pixel_source.load_rgb:
                ...
                vis_timestep = np.linspace(
                    0,
                    sum(dataset.num_img_timesteps.values()),
